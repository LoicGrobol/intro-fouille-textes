...
\begin{frame}[fragile=singleslide]{Annotation}
    ...
    \begin{figure}
        ...
        \begin{tikzpicture}[>=stealth, true scale=0.80]
            ...
            \node[data, below=.6cm of prog, draw=highlight8, text=highlight8] (res) {Ensemble des étiquettes};
            \draw[->] (in) -- (prog);
            \draw[->] (prog) -- (out);
            \draw[->] (res) -- (prog);
        \end{tikzpicture}
...
\begin{frame}[fragile]{Extraction d'informations}
    ...
    \begin{figure}
        ...
        \begin{tikzpicture}[>=stealth, true scale=0.80]
            ...
            \node[data, right=1cm of prog, draw=highlight6, text=highlight6] (out) {Champs remplis};
            \node[data, below=.6cm of prog, draw=highlight8, text=highlight8] (res) {Champs à remplir};
            \draw[->] (in) -- (prog);
            \draw[->] (prog) -- (out);
            \draw[->] (res) -- (prog);
        \end{tikzpicture}
...
\begin{frame}[fragile=singleslide]{La RI comme classification}
    ...
    \begin{figure}
        % \tikzset{external/export=true}
        \begin{tikzpicture}[>=stealth, true scale=0.75]
            \node[data, draw=highlight4, text=highlight4] (in) {Requête};
            \node[program, right=2cm of in, text width=16ex] (prog) {Programme de classification};
            \node[data, right=2cm of prog, draw=highlight6, text=highlight6] (out) {Documents pertinents};
            \node[above left=0.25cm and 0.25cm of prog.north west] (ri) {RI};
            \draw ($(prog.north west)+(-1.5cm, 0.75cm)$) -- ($(prog.north east)+(1.5cm, 0.75cm)$) -- ($(prog.south east)+(1.5cm, -1.25cm)$) -- ($(prog.south west)+(-1.5cm, -1.25cm)$) -- cycle;
            \node[data, below=1.5cm of prog, draw=highlight8, text=highlight8] (res) {Corpus};
