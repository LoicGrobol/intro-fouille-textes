...
\begin{frame}[fragile=singleslide]{Exemples}
    ...
    \begin{itemize}
        ...
        \begin{figure}
            \tikzset{external/export=true}
            \begin{tikzpicture}[fill opacity=0.5, true scale=0.5, text opacity=1]
                \draw[name path=D]
                    (-6,-2) rectangle (6,2)
                    node [above left] {$D$};
                \draw[alt={<1,6>{fill=highlight2, text=highlight2}{}}]
                    (-1, -0.25) circle [x radius=2, y radius=1, rotate=20]
                    node[below left=1.3 and .15] {$P$ };
                \draw[alt={<1,6>{fill=highlight4, text=highlight4}{}}]
                    (1, 0) circle [x radius=2, y radius=1.7]
                    node[above right=1.5 and 0.1] {$R$};
            \end{tikzpicture}
        \end{figure}
    \end{itemize}
\end{frame}
