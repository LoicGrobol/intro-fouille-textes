...
\begin{frame}[fragile]{Validation croisée}
    ...
    \begin{center}
        \begin{tikzpicture}[scale=0.7]
            \foreach \x in {2, ..., 11}
                \draw[alt=<\x>{fill=highlight6}{alt=<1>{}{fill=highlight0}}]
...
\begin{frame}[fragile=singleslide]{Évaluation : préliminaires}
    \begin{figure}
        \begin{tikzpicture}[fill opacity=0.5, scale=0.75, every node/.style={transform shape}]]
            ...
            \filldraw[fill=highlight3]
                (-1, 0) circle [x radius=2, y radius=1, rotate=30]
                node[below left=1.3 and .15, text=highlight3] {$P$ : documents pertinents};
            \filldraw[fill=highlight8]
                (1, 0) circle [x radius=2, y radius=1.5]
                node[above right=1.5 and 0.1, text=highlight8] {$R$ : documents retournés};
        \end{tikzpicture}
