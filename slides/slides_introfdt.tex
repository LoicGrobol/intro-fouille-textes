...
\begin{frame}{Similarité : exemple de travail}
    ...
    \begin{itemize}
        ...
        \item On va pour se forger un intuition géométrique se restreindre au cas de la dimension $2$
    \end{itemize}
\end{frame}
...
\begin{frame}[fragile=singleslide]{Similarités : exemple de travail}
    ...
    \column{0.2\textwidth}
        \begin{figure}
            \begin{tabu} to .8\linewidth {*{3}{X[m, c]}}
                        & $a_1$ & $a_2$\\
                $t_1$   & $3$ & $1$\\
                $t_2$   & $1$ & $2$\\
             \end{tabu}

             Soit $t_1=\tvcord{3, 1}$ et $t_2=\tvcord{1, 2}$
        \end{figure}
    \column{0.8\textwidth}
        \begin{figure}
            \tikzset{external/export=true}
...
% TODO: distances comparisons : show distorted diagrams, + x→||u-v||_x curve
    ...
    \begin{columns}
        ...
        \begin{figure}
            ...
            \begin{tikzpicture}[background rectangle/.style={fill=gray}, show background rectangle, true scale=0.7]
                ...
                \draw[highlight0, line width=2pt] (0, 0) -| (5, 5);
        ...
        \column{.5\textwidth}
            \begin{align}
                \norm{t_2-t_1}_1
                    ...
                    &= \textcolor{highlight0}{\abs{v_{2,1}-v_{1,1}}}+\textcolor{highlight6}{\abs{v_{2,2}-v_{1,2}}}\\
                    &= \textcolor{highlight0}{\abs{1-3}}+\textcolor{highlight6}{\abs{2-1}}\\
                    &= 2+1\\
                    &= 3
            \end{align}
        \column{.5\textwidth}
            \begin{figure}
                ...
                \begin{tikzpicture}[true scale=0.7]
                    \draw [help lines, xstep=1cm, ystep=1cm] (0, 0) grid (5.4, 4.4);
                    ...
                    \draw [->, highlighta, line width=2pt] (0,0) -- (3,1) node[midway, below right] {$t_1$};
                    \draw [->, highlight3, line width=2pt] (0,0) -- (1,2) node[midway, right] {$t_2$};
                    \draw [<->, highlight0] (1, 2) --  (3, 2) node [midway, above] {$\abs{1-3}$};
...
\begin{frame}{Distance eulérienne}
    \begin{block}{Définition (distance eulérienne)}
        On appelle \emph{distance eulérienne} entre $t_1$ et $t_2$ le nombre
        \begin{equation}
            \norm{t_2-t_1}_2 = √{∑_{k=1}^n(v_{2,k}-v_{1,k})²}
        \end{equation}
    \end{block}

    C'est la distance usuelle dans le plan.
    ...
    %             \draw [->, highlighta, line width=2pt] (0,0) -- (3,1) node[midway, above] {$t_1$};
    %             \draw [->, highlight3, line width=2pt] (0,0) -- (1,2) node[midway, above left] {$t_2$};
    %             \draw [<->] (1, 2) --  (3, 1)  node [midway, above right] {$√{5}$};
    %         \end{tikzpicture}
