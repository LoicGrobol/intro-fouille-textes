...
\begin{frame}{Loi de Zipf}
    ...
    \end{block}
    \vskip0pt plus 1fill
    {\tiny 1. Georges Zipf (1902-1950)\\2. d'après la légende}
\end{frame}
...
\begin{frame}{Loi de Zipf : observations}
    \begin{figure}
        \tikzset{external/export=true}
        \begin{tikzpicture}
            \begin{axis}[xmin=0, ymin=0, xmax=20, ymax=50,
                         width=0.9\textwidth,
                         height=0.8\textheight,
                         xlabel={Rang},
                         ylabel={Fréquence d'apparition},
                         title={Loi de Zipf : $y=\frac{K}{x}$}]
                \addplot[highlight0, domain=0.1:20, samples=1001] {20/x};
            \end{axis}
        \end{tikzpicture}
        \caption{Courbe d'une loi de Zipf}
    \end{figure}
\end{frame}
    ...
    \begin{figure}
        ...
        \begin{tikzpicture}
            \begin{axis}[xmin=0, xmax=10000, ymin=0,
                ...
                \addplot[highlight0, domain=0:10000, samples=1001] {40*x^0.5};
            \end{axis}
        \end{tikzpicture}
...
\begin{frame}{Lien entre les lois de Zipf et Heaps}
    Sous des hypothèses assez légères, on peut déduire la loi de Heaps de la loi de Zipf.
...
\begin{frame}{choix des attributs}
    % TODO: better format
    Objectif : transformer les documents à traiter en fichiers tabulaires pour les passer à un classifieur.

    Problème : quel choix d'attributs (colonnes) ?
    \pause

    Solution : ça dépend des cas !
    \pause

    Mais on peut identifier plusieurs catégories élémentaires (air connu)
\end{frame}

\begin{frame}[fragile=singleslide]{Caractères}
    Puisqu'un texte brut est une séquence de caractère : utilisons comme attribut la fréquence de chaque caractère
...
\begin{frame}{$n$-grammes}
    ...
    \begin{itemize}
        \item Quels sont les $1$-grammes ?
        \item Quels sont les bigrammes ?
        \item Quels sont les trigrammes ?
        \item En général, pour chaque, combien de possibilités ?
    \end{itemize}
