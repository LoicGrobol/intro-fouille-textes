...
\usepackage{tikz}
	...
	\usetikzlibrary{positioning, calc, shapes, shapes.multipart, fit}
...
\begin{frame}{Caractérisation}
    ...
    % \begin{tikzpicture}
    %     \draw[->] (0,0) node[draw, circle] (in) {Entrée} -- node[draw, right=2cm of in, text width=10ex] (prog) {Programme réalisant la tâche} -- node[circle, draw, right=2cm of prog] {Résultat};
    % \end{tikzpicture}
\end{frame}
