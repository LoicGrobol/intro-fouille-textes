    ...
    \begin{figure}
        ...
        \begin{tikzpicture}
            \begin{axis}[xmin=0, xmax=800, ymin=0, ymax=600000,
                ...
                \addplot[highlighta, domain=100:310, samples=200] {9.133693238227375E-7*x^6-0.0015210072640020665*x^5+0.9901617434548338*x^4-326.9727250825201*x^3+58224.97189714596*x^2-5327933.125949258*x+196687335.202412};
                \addplot[highlighta, domain=594:606, samples=200] {9.133693238227375E-7*x^6-0.0015210072640020665*x^5+0.9901617434548338*x^4-326.9727250825201*x^3+58224.97189714596*x^2-5327933.125949258*x+196687335.202412};
            \end{axis}
        \end{tikzpicture}
...
\begin{frame}[fragile]{Probabilités conditionnelles}
        ...
        \onslide<10-13>
            ...
            \begin{figure}
                ...
                \begin{tikzpicture}
                    ...
                    \path[fill=highlighta!30, fill opacity=0.5, visible on=12] (-0.5, -0.5)  -- (4.25, 4.25) -- (-0.5, 4.25) -- cycle;
                    \path[fill=highlight6!30, fill opacity=0.5, visible on=12] (-0.5, -0.5)  -- (5.25, -0.5) -- (5.25, 4.25) -- (4.25, 4.25) -- cycle;
                    \draw[highlight3, visible on={11-13}] (-0.5, -0.5)  -- (4.25, 4.25);
                \end{tikzpicture}
...
% TODO: figs
\begin{frame}{Notion de marge}
    Intuitivement, pour espérer qu'un classifieur linéaire sache généraliser hors de l'ensemble d'entraînement, il faut qu'il sépare les classes le plus largement possible.

    \begin{block}{Définition (marge)}
        Étant donné à problème à deux classes, $C$ et $C'$, on appelle \emph{marge} d'un hyperplan $H$ le nombre
        \begin{equation}
            M(H) = \min_{x∈C}d(x, H) + \min_{x'∈C'}d(x', H)
        \end{equation}
    \end{block}

    L'hyperplan recherché pour établir un SVM sera celui pour lequel la marge (sur l'ensemble d'entraînement) sera la plus grande.

    {\footnotesize D'où \emph{séparateur à \alert{vaste marge}}.}
\end{frame}
...
\begin{frame}{Vecteurs de support}
    ...
    \begin{equation}
        M(H) = \min_{x∈C}d(x, H) + \min_{x'∈C'}d(x', H)
    \end{equation}
    Les points $x$ et $x'$ pour lesquels les minima sont atteints  sont appelés \emph{vecteurs de support}
    % TODO: fig
\end{frame}
...
\begin{frame}{Problèmes non-séparables}
    En général, les problèmes réels ne ont pas linéairement séparables.
    Plusieurs méthodes existent pour y adapter les SVM.
    \begin{description}
        \item[Marge douce] On remplace la fonction de marge $M$ par une fonction de la forme
            \begin{equation}
                L(H) = M(H) - C(H)
            \end{equation}
            où $C(H)$ est une pénalité pour les exemples mal classés par $H$.
            On choisit comme précédemment l'hyperplan $H$ qui maximise $L(H)$.
        \item[\emph{Kernel trick}]
    \end{description}
\end{frame}

\begin{frame}{\emph{Kernel trick}}
    Le \emph{Kernel trick} consiste à transformer un problème non-linéairement séparable en un problème linéairement séparable dans un espace de dimension supérieure.

    % TODO: fig

    En réalité, le problème en dimension supérieure est souvent lui aussi non-linéairement séparable, et on combine en général le \emph{Kernel trick} avec une marge douce.
\end{frame}
