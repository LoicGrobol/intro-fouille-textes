...
\begin{frame}{Relations entre tâches}
...
\begin{frame}[fragile]{Décomposition en tâches élémentaires}
    ...
    \only<1>{
        \begin{figure}
            \tikzset{external/export=true}
            \begin{tikzpicture}[true scale=0.5]
                \node[data] (in1) {Entrée};
                ...
                \node[program, right=1cm of out1, text width=12ex] (prog2) {Programme élémentaire};
                \node[data, right=2cm of prog2] (out2) {Résultat};
                \draw[->] (out1) -- (prog2);
                \draw[->] (prog2) -- (out2);

                \draw ($(prog1.north west)+(-1cm, 1cm)$) rectangle ($(prog2.south east)+(1cm, -1cm)$);
            \end{tikzpicture}
            \caption{Exemple de tâche complexe}
        \end{figure}
    }
...
\begin{frame}{Q/R via RI}
    La première reformulation possible utilise un système de \alert{RI}
    ...
    \begin{figure}
        ...
        \begin{tikzpicture}[true scale=0.6]
            ...
            \fill (split) circle[radius=2pt];
            \draw (in) -- (split);

            \node[program, above right=0.5cm and 1cm of split] (term) {Terminologie};
            \node[data, right=1cm of term] (words) {Mots};
            \node[program, right=1cm of words, accent on=<1>] (ri) {RI};
            \node[data, right=1cm of ri] (docs) {Docs};
            \node[program, right=1cm of docs] (ei) {EI};
            \node[coordinate, below right=0.5cm and 1cm of ei] (gather) {};
            \node[data, right=1cm of gather] (answer) {Réponse};
            \draw[->] (split) |- (term);
            \draw[->] (term) -- (words);
            \draw[->] (words) -- (ri);
            \draw[->] (ri) -- (docs);
            ...
            \node[program, below right=0.5cm and 1cm of split] (classif) {Classifieur};
            \node[data, below=1cm of ei] (field) {Champs};
            \draw[->] (split) |- (classif);
            \draw[->] (classif) -- (field);
            \draw[->] (field) -- (ei);
            ...
            \node[data, below=3cm of ri] (corpus) {Corpus};
            \draw[->] (corpus) -- (ri);
        \end{tikzpicture}
