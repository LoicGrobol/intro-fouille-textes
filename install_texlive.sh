...
tlmgr install $(cat **/*.tex | sed -n 's/^[^%]*\\usepackage[^{]*{\([^}]*\)}.*$/\1/p' | sed 's/,\s*/ /p' | paste -sd ' ' -)\
              latexmk oberdiek amsmath tools titlesec caption pgf hyph-utf8 amscls bbm-macros varwidth ms ucharcat xits libertine url doclicense xifthen ifmtarg carlisle shadethm biber logreq xstring etextools etoolbox etex-pkg ec cm-super textpos
